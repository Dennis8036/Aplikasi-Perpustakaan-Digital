...
public function dashboard()
{
    if (session()->get('id_level') > 0) {
        ...
        // Hitung total buku
        $data['total_buku'] = $model->db->table('tb_buku')->countAllResults();
        $data['total_user'] = $model->db->table('tb_user')->countAllResults();
        $data['total_kategori'] = $model->db->table('tb_kategoribuku')->countAllResults();
        $data['total_ulasan'] = $model->db->table('tb_ulasanbuku')->countAllResults();
        $data['total_koleksi'] = $model->db->table('tb_koleksipribadi')->countAllResults();

        // Hitung total peminjaman dengan status "Dipinjam"
        $data['total_dipinjam'] = $model->db->table('tb_peminjaman')
            ->where('statuspeminjaman', 'Dipinjam') // Filter berdasarkan status "Dipinjam"
            ->countAllResults();

            // Ambil 5 pengguna teraktif berdasarkan peminjaman terbaru
$data['pengguna_teraktif'] = $model->db->table('tb_peminjaman')
    ->select('tb_user.username, COUNT(*) as total_peminjaman')  // Pilih username dan hitung peminjaman
    ->join('tb_user', 'tb_user.id_user = tb_peminjaman.id_user')  // Join dengan tb_user
    ->groupBy('tb_user.id_user')  // Grouping berdasarkan id_user untuk mendapatkan total peminjaman
    ->orderBy('total_peminjaman', 'DESC')  // Urutkan berdasarkan total peminjaman
    ->limit(5)  // Ambil 5 pengguna teraktif
    ->get()
    ->getResult();



        // Log aktivitas dan render view
        ...
        public function aksi_e_setting()
{
 ...
 public function user()
        ...
        public function aksi_t_user()
    {
        ...
        $namalengkap = $this->request->getPost('nl');
